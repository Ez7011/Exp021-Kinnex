...
file3="collapse_isoforms.flnc_count_tab.txt"

sed 's/,/\t/g' "$EmmaPath/$file1" > "$EmmaPath/$file3"

cut -f1 "$EmmaPath/$file3” > "$EmmaPath/flnc_ids.txt"
...
awk 'FNR==NR {ids[$1]; next} $1 in ids' "$EmmaPath/common_ids.txt" "$EmmaPath/$file3” > "$EmmaPath/filtered_flnc.txt" 
